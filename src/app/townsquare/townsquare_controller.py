...
class TownsquareQueries:
    ...
    @field
    async def townsquare_by_id(self, townsquare_id: UUID) -> TownsquareType:
...
class TownsquareMutations:
    ...
    @mutation
    async def update_townsquare(self, townsquare_id: UUID, updated_townsquare: TownsquareUpdateIn) -> TownsquareType:
        townsquare = await townsquare_service.update_townsquare(townsquare_id, **vars(updated_townsquare))
        return TownsquareType.from_node(townsquare)

    @mutation
    async def delete_townsquare(self, townsquare_id: UUID, confirmation: bool = False) -> str:
        if not confirmation:
